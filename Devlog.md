...
- **Agora cost-exposure mitigations (gate before any production rollout).** Full diagnosis in the 2026-06-08 audit entry above; Phase 1 closeout in the 2026-06-11 entry. TL;DR: clean-exit paths are solid but idle/forgotten/backgrounded cases burn billable minutes invisibly. Six sized line items, ordered by ROI:
  ...
  - ~~**(P1, ~3 hours)** Headset HMD-worn-state idle detection.~~ **Shipped 2026-06-11** as `UHeadsetPresenceMonitor` (new C++ component, ~120 LOC) + `USignalingSubsystem::EmitHeadsetEnd` exposed BP-callable + new `USignalingSubsystem::RequestSessionResume` BP-callable. The Pico-fallback knob ended up as `bTreatUnknownAsWorn` (conservative default `true`, flip to `false` + raise `IdleThresholdSeconds` for Pico-dominant deployments). **API correction:** `GetHMDWornState()` is declared on `IHeadMountedDisplay`, not on `IXRTrackingSystem` as this audit originally cited — the component routes through `GEngine->XRSystem->GetHMDDevice()->GetHMDWornState()`. BP wiring on `BP_VRPawn` is the only remaining step (~30 min) — drag the component onto the pawn, wire `OnHeadsetIdleStarted` to `EmitHeadsetEnd → Leave Channel → Stop Video Pump`, wire `OnHeadsetIdleEnded` to `RequestSessionResume`. The resume side falls out of the existing Phase 6D `OnAgoraChannelChanged` graph — no new BP graph branches needed beyond the two new event subscriptions.
