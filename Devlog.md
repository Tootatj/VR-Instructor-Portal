...
- **Agora cost-exposure mitigations (gate before any production rollout).** Full diagnosis in the 2026-06-08 audit entry above. TL;DR: clean-exit paths are solid but idle/forgotten/backgrounded cases burn billable minutes invisibly. Six sized line items, ordered by ROI:
  - **(P1, 5 min, manual)** Agora console billing alert at $X/month threshold. Doesn't prevent anything — turns "discovered from monthly invoice" into "alerted before damage compounds." Do this before deploying anywhere a real headset can reach.
  - **(P1, ~3 hours)** Headset HMD-worn-state idle detection. UE exposes `IXRTrackingSystem::GetHMDWornState()` (`Worn` / `NotWorn` / `Unknown`). BP timer on `BP_VRPawn` polls every 30s; after 2 min of `NotWorn`, leave the Agora channel + emit `headset:end` + put the BP in a "waiting for headset to be put back on" state that re-joins automatically on `Worn`. Eliminates the dominant cost path (Quest's proximity sleep doesn't stop the app — just the renderer — so without this we keep publishing into a sleeping headset indefinitely). Pico's worn-state reporting is less reliable; may need a `bUsePicoFallback` config flag with a longer timeout + accelerometer-quiet-window fallback.
  - **(P1, ~1 hour)** Browser tab `visibilitychange` → unsubscribe from tiles. In `grid.js`, listen for `document.visibilityState === 'hidden'` and call `client.unsubscribe(user, 'video')` for every subscribed tile; reverse on `'visible'`. Re-subscription latency on tab focus is ~500ms, imperceptible to the instructor. Saves audience minutes for the common "instructor switches to another tab and forgets" case (~$1.70/day per forgotten 6-tile grid view at Agora's current rates).
  ...
  Recommended phasing: P1 (~4-5h total) before any production rollout. P2 (~6h total) before scaling beyond the first paying customer. P3 only if cost analysis justifies it.
